...
void poly::addMono(class mono & monoNew){
	...
	cout << "Cannot add mono @ addMono" <<endl;
    exit(EXIT_FAILURE);
}
int poly::Degree(){
    setDegree();
...
void bounds::setAll(const vector<double>& newLo, const vector<double>& newUp){
    lower = newLo; 
    upper = newUp; 
}
...
int polysystem::polyIsBinary(int nop){
	if(nop<this->numsys()){
		...
		cout<<"error@polysystem::polyIsBinary :: Nop'th polynomial does not exist"<<endl;
		exit(EXIT_FAILURE);
	}
	return -1;
}
...
int polysystem::polyIsSquareOne(int nop){
	if(nop<this->numsys()){
		...
		cout<<"error@polysystem::polyIsSquareOne :: Nop'th polynomial does not exist"<<endl;
		exit(EXIT_FAILURE);
	}
	return -1;
}
...
int polysystem::polyIsComplimentarity(int nop, class sup & czSup, double & Coef) {
    if(nop<this->numsys()){
        ...
        cout<<"error@polysystem::polyIsComplimentarity :: Nop'th polynomial does not exist"<<endl;
        exit(EXIT_FAILURE);
    }
    return NO;
}
...
void polysystem::setMinOrMax(int val){
	if(val != 1 && val != -1){
		cout << "# Error: minOrmax should be 1 or -1."<< endl;
		exit(EXIT_FAILURE);
	}else{
		minOrmax = val;
	}
